...
<div class="container-fluid page__container">
    ...
    <div class="card card-form">
        <div class="row no-gutters">
            ...
            <form id="category_form" action="{{ route('rooms.update', $room->id ) }}" method="post">
                {{ csrf_field() }}
                @csrf
                @method('PATCH')
                <div class="form-group  {{ $errors->has('name') ? 'has-error' : '' }} ">
                    ...
                    <input name="name"
                            maxlength="21"
                            value="{{ old('name', optional($room ?? null)->name) }}" required
                            id="title"
                           type="text"
                           class="form-control"
                ...
                <div class="form-group  {{ $errors->has('price') ? 'has-error' : '' }} ">
                    ...
                    <input name="price"
                            maxlength="21"
                            value="{{ old('price', optional($room ?? null)->price) }}" required
                            id="title"
                           type="text"
                           class="form-control"
