@forelse($products as $key => $product)
   <tr>
      ...
      <td>
         {{ $product->is_off == 1 ? number_format($product->sale_price - $product->off_price) : number_format($product->sale_price) }}
      </td>
      ...
      <td class="smaller-80">
         @foreach ($product->colors as $color)
            <span style=" background: {{ $color->color_code }} " class="label label-sm">{{ $color->color_name }}</span>
         @endforeach
      </td>
      ...
      <td>
         <div class="hidden-sm hidden-xs btn-group">
            ...
            <a class="btn btn-xs btn-info bolder" title="show product"
               href="{{ route('product.show', $product->product_id) }}"><i class="ace-icon fa fa-eye bigger-120"></i>
            </a>
         </div>
