...
//        $Product_availability = number_format((float)$percantage , 2 , '.' ,'');
        ...
        $popular_products = $this->product->findOrFail($popular_product,
            ['status', 'product_id', 'product_name', 'sale_price', 'off_price', 'is_off','created_at']);

        /*--------------- feedback:------------------*/
        $feedbacks = Feedback::where('status', 0)->take(4)->get();
        $feedbacksCount = Feedback::where('status', 0)->count();

        return view('admin.dashboard.dashboard', compact(
            'discounted_products', 'available_products', 'product_news', 'new_users', 'employees',
            'order_news', 'order_sent', 'order_not_complete', 'popular_products','payment_failed','payment_success','payment_week','feedbacks','feedbacksCount'));
    }
